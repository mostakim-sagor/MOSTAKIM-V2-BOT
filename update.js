...
async function backup(configValue) {
    ...
    mkdirSync(process.cwd() + "/tmp/main")
        if (existsSync('./MOSTAKIM')) copySync('./MOSTAKIM', './tmp/MOSTAKIM');
        ...
        if (existsSync(`./${configValue.DATABASE.sqlite.storage}`)) copySync(`./${configValue.DATABASE.sqlite.storage}`, `./tmp/${configValue.DATABASE.sqlite.storage}`);
}
...
function install () {
    ...
    copySync(process.cwd() + '/tmp/main/MOSTAKIM-V2-BOT-main/', './');
    return;
}
...
async function finish(configValue) {
        ...
        if (existsSync(`./tmp/${configValue.DATABASE.sqlite.storage}`)) copySync(`./tmp/${configValue.DATABASE.sqlite.storage}`, `./${configValue.DATABASE.sqlite.storage}`);
