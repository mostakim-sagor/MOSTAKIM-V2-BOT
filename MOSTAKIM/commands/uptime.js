  ...
  run: async function ({ api, event }) {
    ...
    try {
      ...
      const pingStatus =
        ping < 100 ? "⚡ Ultra Fast" :
        ping < 200 ? "🚀 Stable"    :
        ping < 400 ? "⚠️ Normal"    : "🐢 Slow";

      const status =
        usedPercent < 70 ? "✅ SYSTEM STABLE" :
        usedPercent < 90 ? "⚠️ HIGH LOAD"     : "⛔ CRITICAL";

      const finalMsg =
`╭───〔⚙️ 𝐒𝐘𝐒𝐓𝐄𝐌 𝐒𝐓𝐀𝐓𝐔𝐒 ⚙️〕───╮
│ 🤖 𝗕𝗢𝗧: ${botName}
│ 🕐 𝗦𝗧𝗔𝗥𝗧: ${startTime.toLocaleString()}
...
╰───────────────────────`;
      ...
      await api.sendMessage("❌ Error generating uptime report.", event.threadID);
    }
  }
};
