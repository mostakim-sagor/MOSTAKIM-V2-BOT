module.exports = function({ api, models }) {
        ...
        return (event) => {
                // ── noPrefix: bot admin/dev can use commands without prefix ──
                let evt = event;
                if (
                        global.config.NOPFX &&
                ...
                switch (evt.type) {
                        ...
                        case "message_reply":
                        case "message_unsend":
                                handleCreateDatabase({ event: evt });
                                handleCommand({ event: evt });
                                handleReply({ event: evt });
                                handleCommandEvent({ event: evt });
                                break;
                        case "event":
                                handleEvent({ event: evt });
                                break;
                        case "message_reaction":
                                handleReaction({ event: evt });
                                break;
                        default:
                                break;
                }
